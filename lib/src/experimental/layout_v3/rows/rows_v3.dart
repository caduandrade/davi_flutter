...
class RowsV3<ROW> extends StatelessWidget {
  ...
  const RowsV3({Key? key, required this.layoutSettings, required this.model}) : super(key: key);

  final EasyTableModel<ROW>? model;
  final RowsLayoutSettings layoutSettings;
  ...
  Widget build(BuildContext context) {
    ...
    if(model!=null) {
      final int last = layoutSettings.firstRowIndex+layoutSettings.visibleRowsLength;
      for (int i = layoutSettings.firstRowIndex; i < last && i<model!.visibleRowsLength;i++) {
        RowV3 row = RowV3(rowIndex: i, row: model!.visibleRowAt(i));
        children.add(RowsLayoutChildV3(index: i, child: row));
      }

      //TODO use correct color
      RowsPaintingSettings paintSettings=RowsPaintingSettings(divisorColor: Colors.green);
      return RowsLayoutV3<ROW>(layoutSettings:layoutSettings, paintSettings:paintSettings, children: children);
    }
