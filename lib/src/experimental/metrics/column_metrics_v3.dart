...
class ColumnMetricsV3<ROW>{
  ...
  ColumnMetricsV3({required this.column,required  this.width,required  this.offset,required  this.pinStatus});
  ...
  static List<ColumnMetricsV3<ROW>> columnsFit<ROW>(
    ...
    for (int i = 0; i < model.columnsLength; i++) {
      ...
      list.add(ColumnMetricsV3<ROW>(column: column, width: width, offset: offset, pinStatus: PinStatus.unpinned));
    ...
    for(PinStatus pinStatus in PinStatus.values) {
      for (int i = 0; i < model.columnsLength; i++) {
        ...
        if ((pinStatus == PinStatus.unpinned && column.pinned == false) ||
          ...
          list.add(ColumnMetricsV3<ROW>(column: column, width: column.width, offset: offset, pinStatus:pinStatus));
